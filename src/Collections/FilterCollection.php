...
use Signal\Core\Reporting\Filters\FilterInterface;
...
class FilterCollection implements Countable, IteratorAggregate
{
    ...
    public function indexOf(FilterInterface $element): false|int
    {
    ...
    public function add(FilterInterface $item): void
    {
        $this->innerCollection->add($item);
    }

    public function remove(FilterInterface $item): void
    {
    ...
    public function contains(FilterInterface $item): bool
    {
    ...
    public function first(Closure|null $callback = null): FilterInterface|null
    {
    ...
    private static function assertArray(array $items): void
    {
        foreach ($items as $item) {
            if (false === ($item instanceof FilterInterface)) {
                throw new InvalidArgumentException('All items must be of type: ' . FilterInterface::class);
            }
        }
    }
}
