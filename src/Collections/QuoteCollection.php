 ...
 */
class QuoteCollection implements Countable, IteratorAggregate
{
    ...
    public function indexOf(QuoteInterface $element): false|int
    {
    ...
    public function add(QuoteInterface $item): void
    {
        $this->innerCollection->add($item);
    }

    public function remove(QuoteInterface $item): void
    {
    ...
    public function first(Closure|null $callback = null): QuoteInterface|null
    {
    ...
    private static function assertArray(array $items): void
    {
        foreach ($items as $item) {
            if (false === ($item instanceof QuoteInterface)) {
                throw new InvalidArgumentException('All items must be of type: ' . QuoteInterface::class);
            }
        }
    }
}
